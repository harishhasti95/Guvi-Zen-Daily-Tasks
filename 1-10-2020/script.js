...
getCards().then(function(data){
    ...
    for(c=0;c<=len;c=c+3){
        ...
        var b = (c+1);
        var c = (c+2);
        var a_ = create_row_cards(a,data[a].name,data[a].flag,data[a].capital,data[a].region,data[a].alpha3Code,data[a].latlng);
        var b_ = create_row_cards(b,data[b].name,data[b].flag,data[b].capital,data[b].region,data[b].alpha3Code,data[b].latlng);
        var c_ = create_row_cards(a,data[c].name,data[c].flag,data[c].capital,data[c].region,data[c].alpha3Code,data[c].latlng);
        var rrr = create_row();
        rrr.appendChild(a_);
        rrr.appendChild(b_);
        rrr.appendChild(c_);
...
function create_row_cards(id, name, flag, capital, region, ccode,latlngarr){
    ...
    body_div_lat = document.createElement('div');
    body_div_lat.setAttribute('class','card-body');
    body_div_lat.style.padding = '5px';
    body_div_lat.innerHTML = 'Latitude: ' + latlngarr[0];

    body_div_long = document.createElement('div');
    body_div_long.setAttribute('class','card-body');
    body_div_long.style.padding = '5px';
    body_div_long.innerHTML = 'Longitude: ' + latlngarr[1];
    ...
    body_div.appendChild(body_div_lat);
    body_div.appendChild(body_div_long);
    ...
    button.onclick = async function(){
        var api_key = '63b6fb08027d810af13b91538d898098';
